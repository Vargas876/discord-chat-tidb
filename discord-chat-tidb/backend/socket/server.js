...
io.on('connection', (socket) => {
  ...
  socket.on('send-message', async (data) => {
    ...
    try {
      ...
      // Enviar a todos los usuarios en la conversación (incluyendo el remitente)
      io.to(conversation_id).emit('new-message', messageData);

      console.log(`💬 Mensaje enviado a sala ${conversation_id}: ${content.substring(0, 50)}...`);
    } catch (error) {
